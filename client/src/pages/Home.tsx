...
export default function Home() {
  ...
  const shells = Array.from({ length: 20 }).map((_, i) => ({
    id: i,
  ...
  const fallingVerses = Array.from({ length: 15 }).map((_, i) => {
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ x: shellX, y: shellY }} className="absolute inset-0 pointer-events-none">
        {shells.map(shell => (
          <div
            key={`shell-${shell.id}`}
          ...
          >
            ...
            <div className="oriental-border rounded-2xl bg-card/60 backdrop-blur-sm p-8 md:p-12">
              <AnimatePresence mode="wait">
                ...
                >
                  <p
                    className="font-arabic text-3xl md:text-5xl leading-relaxed md:leading-loose text-shadow-glow text-hover-halo"
